namespace AngleSharp.Css.Dom
{
    ...
    public static class CssValueExtensions
    {
        ...
        public static long AsArgb(this ICssValue value)
        {
            if (value is Color res)
            {
        ...
        {
            if (!(value is Length length) || length.Type != Length.Unit.Percent) return 0.0;
            if (length.Type == Length.Unit.Percent)
            {
                return (float) (length.Value * 0.01);
            }
            return 0.0;
        }

        public static bool IsPercent(this ICssValue value)
        {
            return value is Length length && length.Type == Length.Unit.Percent;
        }
